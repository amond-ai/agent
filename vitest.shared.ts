 ...
 * path repo-root-relative, which is the only form the sonar scanner resolves.
 *
 * Annotated rather than `as const`: a workspace whose `tsconfig.json` includes its own config
 * files typechecks the object this is spread into, and `CoverageOptions.reporter` is a mutable
 * array that a `readonly` tuple cannot be assigned to. The annotation keeps `provider` narrow
 * without freezing the shape.
 */
